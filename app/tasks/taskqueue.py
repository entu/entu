...
while True:
    ...
    for customer_row in databases.values():
        ...
        last_checked.setdefault(customer_row.get('domain'), datetime.min)
        last_checked.setdefault('_tq:metrics', {'properties_checked': 0, 'time_spent': 0})
        ...
        """ % last_checked[customer_row.get('domain')]
        ...
        for property_row in db.query(sql):
            ...
            sql = """
                ...
                LEFT JOIN property p ON p.entity_id = e.id
                          AND p.is_deleted = 0
                          AND p.value_formula LIKE concat('%%%%.child.%s}%%%%')
                          AND p.changed < "%s"
                RIGHT JOIN property_definition pd ON p.property_definition_keyname = pd.keyname
                          AND pd.formula = 1
                WHERE r.related_entity_id = %s
                  AND r.relationship_definition_keyname = 'child'
                  AND r.is_deleted = 0;
            """ % (property_row.dataproperty, str(property_row.created), property_row.entity_id)
            # print sql
            ...
            last_checked[customer_row.get('domain')] = str(property_row.created)
