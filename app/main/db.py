...
class Entity():
    ...
    def set_rethinkdb_entity(self, entity_id):
        ...
        if r.get('entity_created'):
            e.setdefault('_created', {})['at'] = rethinkdb.iso8601(r.get('entity_created').isoformat())
        ...
        if r.get('entity_changed'):
            e.setdefault('_changed', {})['at'] = rethinkdb.iso8601(r.get('entity_changed').isoformat())
        ...
        if r.get('entity_is_deleted') and r.get('entity_deleted'):
            e.setdefault('_deleted', {})['at'] = rethinkdb.iso8601(r.get('entity_deleted').isoformat())
        ...
        for r2 in self.db.query(sql, mysql_id):
            ...
            if r2.get('property_datatype') == 'string' and r2.get('value_string'):
                value['value'] = u'%s' % r2.get('value_string')
            elif r2.get('property_datatype') == 'text' and r2.get('value_text'):
                value['value'] = u'%s' % r2.get('value_text')
                ...
                value['value'] = rethinkdb.iso8601(r2.get('value_datetime').isoformat())
            ...
            if r2.get('created'):
                value.setdefault('created', {})['at'] = rethinkdb.iso8601(r2.get('created').isoformat())
            if r2.get('created_by'):
                value.setdefault('created', {})['by'] = r2.get('created_by')

            if r2.get('is_deleted') and r2.get('deleted'):
                value.setdefault('deleted', {})['at'] = rethinkdb.iso8601(r2.get('deleted').isoformat())
