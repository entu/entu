...
class Entity():
    ...
    def set_rethinkdb_entity(self, entity_id):
        ...
        if r.get('entity_created'):
            e.setdefault('_created', {})['at'] = time.mktime(r.get('entity_created').timetuple()).to_iso8601()
        ...
        if r.get('entity_changed'):
            e.setdefault('_changed', {})['at'] = time.mktime(r.get('entity_changed').timetuple()).to_iso8601()
        ...
        if r.get('entity_is_deleted') and r.get('entity_deleted'):
            e.setdefault('_deleted', {})['at'] = time.mktime(r.get('entity_deleted').timetuple()).to_iso8601()
        ...
        for r2 in self.db.query(sql, mysql_id):
            ...
            if r2.get('property_datatype') == 'string' and r2.get('value_string'):
                ...
                value['value'] = time.mktime(r2.get('value_datetime').timetuple()).to_iso8601()
            ...
            if r2.get('created'):
                value.setdefault('created', {})['at'] = time.mktime(r2.get('created').timetuple()).to_iso8601()
            if r2.get('created_by'):
                value.setdefault('created', {})['by'] = r2.get('created_by')

            if r2.get('is_deleted') and r2.get('deleted'):
                value.setdefault('deleted', {})['at'] = time.mktime(r2.get('deleted').timetuple()).to_iso8601()
