        ...
        if search != None:
            ...
            for s in search.split(' ')[:5]:
                if not s:
                    continue
                i += 1
                join_parts.append('RIGHT JOIN property AS p%(idx)i ON p%(idx)i.entity_id = e.id RIGHT JOIN property_definition AS pd%(idx)i ON pd%(idx)i.keyname = p%(idx)i.property_definition_keyname AND pd%(idx)i.search = 1' % {'idx': i})
