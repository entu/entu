...
class Entity():
    ...
    def set_rethinkdb_entity(self, entity_id):
        ...
        if r.get('entity_created'):
            e.setdefault('_created', {})['at'] = rethinkdb.iso8601(r.get('entity_created').utcnow().isoformat())
        ...
        if r.get('entity_changed'):
            e.setdefault('_changed', {})['at'] = rethinkdb.iso8601(r.get('entity_changed').utcnow().isoformat())
        ...
        if r.get('entity_is_deleted') and r.get('entity_deleted'):
            e.setdefault('_deleted', {})['at'] = rethinkdb.iso8601(r.get('entity_deleted').utcnow().isoformat())
        ...
        for r2 in self.db.query(sql, mysql_id):
            ...
            if r2.get('property_datatype') == 'string' and r2.get('value_string'):
                ...
                value['value'] = rethinkdb.iso8601(r2.get('value_datetime').utcnow().isoformat())
            ...
            if r2.get('created'):
                value.setdefault('created', {})['at'] = rethinkdb.iso8601(r2.get('created').utcnow().isoformat())
            if r2.get('created_by'):
                value.setdefault('created', {})['by'] = r2.get('created_by')

            if r2.get('is_deleted') and r2.get('deleted'):
                value.setdefault('deleted', {})['at'] = rethinkdb.iso8601(r2.get('deleted').utcnow().isoformat())
        ...
        rethinkdb_entity = list(rethinkdb.table('entity').filter({'_mid': mysql_id}).limit(1).get_field('id'))
        if rethinkdb_entity:
